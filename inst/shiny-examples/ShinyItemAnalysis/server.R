...
function(input, output, session) {
  ...
  observeEvent(input$dataSelect, {
    ...
    dataset$nominal <- data.table(dataNominal)
    dataset$ordinal <- data.table(dataOrdinal)
    dataset$binary <- data.table(dataBinary)
  ...
  observeEvent(input$submitButton, {
      ...
      # loading max/min/cut values for ordinal data
      if (input$data_type == "ordinal"){
        ### minimal values
      ...
      if (input$data_type == "nominal"){
        dataset$ordinal <- mirt::key2binary(dataset$nominal, inputKey)
        dataset$binary <- mirt::key2binary(dataset$nominal, inputKey)
      } else {
        if (input$data_type == "ordinal"){
          dataset$ordinal <- dataset$nominal
          dataset$binary <- mirt::key2binary(dataset$nominal, inputKey)
        } else {
          dataset$ordinal <- dataset$nominal
          dataset$binary <- dataset$nominal
        }
      }
      ...
      # # warning for missing group
      # if (length(dataset$group) == 1 & dataset$group == "missing"){
      ...
      # # warning for missing criterion
      # if (length(dataset$criterion) == 1 & dataset$criterion == "missing"){
      ...
      print(head(dataset$nominal))
      print(head(dataset$ordinal))
      print(head(dataset$binary))
      print(head(dataset$key))
      print(head(dataset$group))
      print(head(dataset$criterion))
  ...
  item_numbers <- reactive({
    if (!input$itemnam){
      nam <- 1:ncol(dataset$ordinal)
    } else {
      nam <- colnames(dataset$ordinal)
    }
    nam
  })
  ...
  item_names <- reactive({
    if (!input$itemnam){
      nam <- paste("Item", 1:ncol(dataset$ordinal))
    } else {
      nam <- colnames(dataset$ordinal)
    }
    nam
  ...
  # DATA ######
  DPdata <- reactive ({
    dataset <- data.table(dataset$binary, dataset$group)
  ...
  observe({
    ...
    itemCount = ncol(dataset$ordinal)
    updateSliderInput(session = session, inputId = "slider_totalscores_histogram",
                      max = itemCount,
                      value = round(median(dataset$total_score, na.rm = T)))
    updateNumericInput(session = session, inputId = "corr_plot_clust",
                       value = 1,
                       max = itemCount)
