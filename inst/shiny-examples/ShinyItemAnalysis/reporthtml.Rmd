...
To cite ShinyItemAnalysis application in publications, please, use:
  ...
  (2018)
  ShinyItemAnalysis: Test and item analysis via shiny. R package version 1.2.9.
...
The total score also known as the raw score is a total number of correct answers. It can be used to compare individual score to a norm group, e.g. if the mean is 12, then individual score can be compared to see if it is below or above this average. Percentile indicates the value below which a percentage of observations falls, e.g. an individual score at the 80th percentile means that the individual score is the same or higher than the scores of 80% of all respondents. Success rate is the percentage of correct answers, e.g. if the maximum points of test is equal to 20 and individual score is 12 then success rate is 12/20 = 0.6, i.e. 60%. The Z-score or also the standardized score is a linear transformation of total score with a mean of 0 and with variance of 1. If X is total score, M its mean and SD its standard deviation then Z-score = (X - M) / SD. The T-score is transformed Z-score with a mean of 50 and standard deviation of 10. If Z is Z-score then T-score = (Z * 10) + 50.
...
`r if (params$irt_type != "none") {"More complex estimates of ability are provided in IRT section."}`
...
`r if (any(params$corr_plot != "")) {if (params$corr_type == "pearson") {"Correlation heat map displays Pearson correlations of items. Pearson correlation coefficient describes linear correlation between two random variables X and Y."}}`

`r if (any(params$corr_plot != "")) {if (params$corr_type == "polychoric") {"Correlation heat map displays polychoric correlations of items. Polychoric/tetrachoric correlation between two ordinal/binary variables is calculated from their contingency table, under the assumption that the ordinal variables dissect continuous latent variables that are bivariate normal."}}`

`r if (any(params$corr_plot != "")) {if (params$corr_type == "spearman") {"Correlation heat map displays Spearman correlations of items. Spearman's rank correlation coefficient describes strength and direction of monotonic relationship between random variables X and Y, i.e. dependence between the rankings of two variables."}}`
...
`r clustmethod_names <- c("None" = "none",
                          ...
                          "median" = "median",
                          "centroid" = "centroid")`
`r if (any(params$corr_plot != "") & params$corr_plot_clustmethod != "none") {paste("Correlation heat map is reordered using hierarchical clustering with", names(clustmethod_names)[clustmethod_names == params$corr_plot_clustmethod], "linkage method. Number of highlighted clusters is", params$corr_plot_numclust, ". ")}`
...
`r if (any(params$corr_plot != "")) {"Scree plot displays the eigenvalues associated with a component or a factor in descending order."}`
...
`r if (params$DDplotDiscType == "RIR") {"Difficulty (red) of items is estimated as percent of respondents who answered correctly to that item. Discrimination (blue) is described by Pearson correlation between item and rest of items. "}`

`r if (params$DDplotDiscType == "RIT") {"Difficulty (red) of items is estimated as percent of respondents who answered correctly to that item. Discrimination (blue) is described by Pearson correlation between item and total score. "}`
...
`r if (params$irt_type == "rasch") {"Model parameters are estimated using marginal maximum likelihood (MML) method. Ability $\\theta$ is assumed to follow normal distribution with freely estimated variance. "}`

`r if (params$irt_type %in% c("1pl", "2pl", "3pl", "4pl")) {"Model parameters are estimated using marginal maximum likelihood (MML) method. Ability $\\theta$ is assumed to follow standard normal distribution. "}`

`r if (params$irt_type != "none") {"## Ability estimates"}`

`r if (params$irt_type != "none") {"Ability is estimated using EAP algorithm and selected IRT model. Relationship between ability estimates (factor scores, F-scores) and standardized total test scores (Z-scores) can be seen on scatter plot below. Table with ability estimates for all respondents can be downloaded in the application. "}`
