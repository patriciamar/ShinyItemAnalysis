...
`r if (params$irt_type != "rasch") {"Ability $\\theta$ is assumed to follow normal distribution with freely estimated variance. "}`

`r if (params$irt_type %in% c("1pl", "2pl", "3pl", "4pl")) {"Ability $\\theta$ is assumed to follow standard normal distribution. "}`
...
`r if (params$irt_type != "none") {"Ability is estimated by selected IRT model. Relationship between ability estimates (factor scores, F-scores) and standardized total test scores (Z-scores) can be seen on scatter plot below. The complete score table for all respondents can be downloaded in the application. "}`
