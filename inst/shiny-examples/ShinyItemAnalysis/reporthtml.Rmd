...
`r if(params$deltaplotCheck){if(params$isGroupPresent){res <- params$DP_text_normal; paste("Detection threshold is ", round(res$thr, 2), ". ", ifelse(res$DIFitems == "no DIF item detected", "**No DIF item detected.**  ", paste("**Items detected as DIF: ", res$DIFitems, ". **  ", sep = "")), sep = "")}}`
`r if(params$isGroupPresent & params$logregCheck) {res <- params$DP_text_normal; paste(ifelse(res$purify, "Item purification was used.", "Item purification was not applied."))}`

```{r delta-plot-output, echo = FALSE}
if (params$deltaplotCheck) {
  if(params$isGroupPresent){
    res <- params$DP_text_normal
...
`r if(params$isGroupPresent & params$multiCheck) {res <- params$model_DDF_print; switch(res$type, "both" = "Both types of DDF tested.", "udif" = "Uniform DDF tested only.", "nudif" = "Non-uniform DDF tested only.")}`
`r if(params$isGroupPresent & params$multiCheck) {res <- params$model_DDF_print; paste(ifelse(any(res$DDFitems == "No DDF item detected"), "**No DDF item detected. **  ", paste("**Items detected as DDF: ", paste(res$DDFitems, collapse = ", "), ". **  ", sep = "")), sep = "")}`
`r if(params$isGroupPresent & params$multiCheck) {res <- params$model_DDF_print; paste(ifelse(res$purification, "Item purification was used.", "Item purification was not applied."), ifelse(is.null(res$p.adjust.method), "No p-value adjustment for multiple comparisons was used.", paste(switch(res$p.adjust.method, "holm" = "Holm's", "hochberg" = "Hochberg's", "hommel" = "Hommel's", "bonferroni" = "Bonferroni", "BH" = "Benjamini-Hochberg", "BY" = "Benjamini-Yekutieli", "fdr" = "Benjamini-Hochberg", "none" = "No"), "p-value adjustment for multiple comparisons was used.")))}`

```{r ddf-multi-output, echo = FALSE}
if (params$multiCheck) {
  if(params$isGroupPresent){
    res <- params$model_DDF_print
...
`r if(params$isGroupPresent & params$multiCheck & !is.null(params$model_DDF_print[[1]])) {"### Characteristic curves of DDF items"}`
