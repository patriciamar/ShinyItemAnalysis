...
`r if (params$isGroupPresent & params$deltaplotCheck){res <- params$DIF_deltaplot_text; paste0("The detection threshold is ", round(res$thr[length(res$thr)], 2), ". ", ifelse(any(res$DIFitems == "no DIF item detected"), "**No DIF item was detected.**", paste0("**Items detected as DIF: ",   paste0(params$itemNames[res$DIFitems], collapse = ", "), ". ** ")))}`
...
`r if (params$isGroupPresent & params$logregCheck) {res <- params$DIF_logistic_print; paste("The detection threshold is ", round(res$thr, 2), ". ", ifelse(any(res$DIFitems == "No DIF item detected"), "**No DIF item was detected. **  ", paste("**Items detected as DIF: ", paste(params$itemNames[res$DIFitems], collapse = ", "), ". **  ")))}`
