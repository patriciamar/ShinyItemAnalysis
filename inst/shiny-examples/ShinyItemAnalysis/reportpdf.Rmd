...
params$wrightMap + theme(text = element_text(size = 16))
```

```{r IRT-equation, echo = FALSE, results = "hide", eval = show_IRT}
IRT_selected_model <- switch(params$irt_type,
...
IRT_equation <- switch(params$irt_type,
                             "rasch" = "$${P}(Y_{ij} = 1|\\theta_{i}, a, b_j) = \\frac{e^{(\\theta_i - b_j)}}{1 + e^{(\\theta_i - b_j)}}$$",
                             "1pl"   = "$${P}(Y_{ij} = 1|\\theta_{i}, a, b_j) = \\frac{e^{a(\\theta_i - b_j)}}{1 + e^{a(\\theta_i - b_j)}}$$",
                             "2pl"   = "$${P}(Y_{ij} = 1|\\theta_{i}, a_j, b_j) = \\frac{e^{a_j(\\theta_i - b_j)}}{1 + e^{a_j(\\theta_i - b_j)}}$$",
                             "3pl"   = "$${P}(Y_{ij} = 1|\\theta_{i}, a_j, b_j, c_j) = c_j + (1 + c_j) \\frac{e^{a_j(\\theta_i - b_j)}}{1 + e^{a_j(\\theta_i - b_j)}}$$",
                             "4pl"   = "$${P}(Y_{ij} = 1|\\theta_{i}, a_j, b_j, c_j, d_j) = c_j + (d_j + c_j) \\frac{e^{a_j(\\theta_i - b_j)}}{1 + e^{a_j(\\theta_i - b_j)}}$$")
```

```{r IRT-equation-text, echo = FALSE, results = "asis", eval = show_IRT}
txt <- ifelse(params$irt_type == "rasch", 
...
```{r IRT-curves-plots-short, echo = FALSE, results = "hide", fig.height = 14.5, fig.width = 11}
...
```{r IRT-curves-plots-long, echo = FALSE, results = "hide", fig.height = 16, fig.width = 11}
if (params$irt_type != "none") {
  if (ncol(params$a) >= 50){
    g1 <- params$irt + 
      ggtitle("Item characteristic curves") + 
...
cat(paste("\\pagebreak # DIF/Fairness analysis", txt))
```
...
DIF is not about total scores! Two groups may have the same distribution of total scores, yet, some item may function differently for the two groups. Also, one of the groups may have a signifficantly lower total score, yet, it may happen that there is no DIF item [(see Martinkova et al., 2017)](https://doi.org/10.1187/cbe.16-10-0307)!
")
```
...
txt <- paste0("For selected cut-score ", params$cutScoregroup, ", the blue part of histograms shows respondents with a total score above the cut-score, the grey column shows respondents with a  total score equal to the cut-score and the red part of histograms shows respondents below the cut-score.")

cat(paste("\\vspace{-2em} ### Histogram of total score by group \n", txt))
```
...
if (dim(tab)[2] == 5){
  colnames(tab) <- c("Stat.", "P-value", "Adj. p-value", "$\\alpha_{\\text{MH}}$", "$\\delta__{\\text{MH}}$")
  print(xtable(tab, 
               digits = c(0, 3, 3, 0, 0, 0), 
               align = c('l', 'r', 'r', 'l', 'r', 'r')),
        booktabs = T, comment = F, tabular.environment = "longtable", floating = F, 
        sanitize.text.function = function(x){x}, add.to.row = add.to.row, hline.after = c(-1, nrow(tab)))
} else {
  colnames(tab) <- c("Stat.", "P-value", "$\\alpha_{\\text{MH}}$", "$\\delta__{\\text{MH}}$")
  print(xtable(tab, 
...
if (dim(tab)[2] == 6){
  ...
  print(xtable(tab, 
        ...
        booktabs = T, comment = F, tabular.environment = "longtable", floating = F, 
        sanitize.text.function = function(x){x}, add.to.row = add.to.row, 
...
eval_DDF_plot <- eval_DDF & !is.null(params$DDF_multinomial_plot[[1]])

if (eval_DDF) res_DDF <- params$DDF_multinomial_print
```
...
```{r DIF-section-DDF-part3, eval = eval_DDF_plot, echo = FALSE, results = 'asis'}
txt <- ifelse(ncol(tab) < 26, "\\pagebreak", "")
cat(paste(txt,
"
...
```{r DIF-section-DDF-plot, echo = FALSE, fig.height = 4.1, fig.width = 11, eval = eval_DDF_plot}
