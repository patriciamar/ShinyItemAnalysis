...
The table below summarizes basic characteristics of total scores including number of respondents, number of complete cases without any missing value, minimum and maximum values, mean, median, standard deviation, skewness, and kurtosis. The skewness for normally distributed scores is near the value of 0 and the kurtosis is near the value of 3.
...
`r if (!is.null(params$corr_plot) | params$validity_check) {"\\pagebreak"}`

`r if (!is.null(params$corr_plot) | params$validity_check) {"# Validity"}`

`r if (!is.null(params$corr_plot)) {"## Correlation structure"}`

`r if (!is.null(params$corr_plot)) {"### Correlation heat map "}`
`r if (!is.null(params$corr_plot)) {if (params$corr_type == "pearson") {"A correlation heat map displays Pearson correlations of items. Pearson correlation coefficient describes linear correlation between two random variables X and Y."}}`

`r if (!is.null(params$corr_plot)) {if (params$corr_type == "polychoric") {"A correlation heat map displays polychoric correlations of items. Polychoric/tetrachoric correlation between two ordinal/binary variables is calculated from their contingency table, under the assumption that the ordinal variables dissect continuous latent variables that are bivariate normal."}}`

`r if (!is.null(params$corr_plot)) {if (params$corr_type == "spearman") {"A correlation heat map displays Polychoric correlations of items. Spearman's rank correlation coefficient describes strength and direction of monotonic relationship between random variables X and Y, i.e. dependence between the rankings of two variables."}}`
...
`r if (!is.null(params$corr_plot) & params$corr_plot_clustmethod != "none") {paste0("A correlation heat map is reordered using hierarchical clustering with ", names(clustmethod_names)[clustmethod_names == params$corr_plot_clustmethod], " linkage method. The number of highlighted clusters is ", params$corr_plot_numclust, ". ")}`

`r if (!is.null(params$corr_plot) & params$corr_plot_clustmethod != "none") {"\\vspace{2em}"}`

```{r corr-plot, fig.align = 'center', echo = FALSE, include = !is.null(params$corr_plot), fig.height = 5}
params$corr_plot
```

`r if (!is.null(params$corr_plot)) {"\\pagebreak"}` 
...
if (params$isCriterionPresent & params$validity_check) {
  ...
  validity_table_interpretation <- paste(
    ...
    ifelse(pval < .05,
      ...
      "The \\textit{p}-value is larger than .05, thus we don't reject the null hypotheses. We cannot conclude that a significant correlation between the total score and criterion variable exists."
    )
  )
}
```
...
print(
  ...
  booktabs = T, comment = F, include.rownames = T, sanitize.colnames.function = identity,
...
if (dim(tab)[2] == 5) {
  ...
  print(
    ...
    sanitize.text.function = identity, # do not sanitize colnames and the content
    sanitize.rownames.function = NULL, # sanitize item names
    add.to.row = add.to.row, hline.after = c(-1, nrow(tab))
  )
} else {
