...
fa_parallel <- function(
  ...
  if (is_corr(Data)) {
    ...
    # real data part
    data_corr <- switch(
      match.arg(cor, c("pearson", "tetrachoric", "polychoric")),
      ...
      "tetrachoric" = tryCatch(
        tetrachoric(Data, na.rm = TRUE, ...)$rho,
        error = function(e) {
          if (max(Data, na.rm = TRUE) > 1L) {
            stop(
              "Tetrachoric correlation requires dichotomous data.", # typo in original psych error
              call. = FALSE
            )
          } else {
            stop(e)
  ...
  get_eigenvals <- function(sim_eigen_list, threshold, n_iter, p) {
    ...
    switch(
      match.arg(threshold, c("mean", "quantile")),
  ...
  cat(
    ...
    switch(
      ...
      "both" = paste0(
        "factors is ",
        n_factors,
        " and the optimal number of principal components is ",
        n_comp,
        "."
      )
    ),
    ...
    switch(
      ...
      "fa" = paste0(
        kaiser_fa,
        ifelse(kaiser_fa > 1L, " factors are", " factor is"),
        " recommended."
      ),
      "pca" = paste0(
        kaiser_pca,
        ifelse(kaiser_pca > 1L, " components are", " component is"),
        " recommended."
      ),
      "both" = paste0(
        kaiser_fa,
        ifelse(kaiser_fa > 1L, " factors", " factor"),
        " and ",
        kaiser_pca,
        ifelse(kaiser_pca > 1L, " components are", " component is"),
        " recommended."
...
#' }
#'
#' @importFrom ggplot2 ggplot aes geom_line geom_point geom_hline annotate scale_x_continuous scale_color_manual theme_bw theme expansion element_line element_rect scale_alpha_manual xlab element_blank element_text coord_cartesian
...
plot.sia_parallel <- function(x, y, ...) {
  ...
  kaiser_boundary <- function(method, show_kaiser = TRUE) {
    if (show_kaiser) {
      positions <- switch(method, "fa" = 0, "pca" = 1, "both" = c(0, 1))
      labels <- switch(
        method,
        "fa" = "Kaiser boundary",
        "pca" = "Kaiser boundary",
  ...
  x |>
    ...
    theme(
      panel.grid.major = element_blank(), # element_line(linewidth = .4),
      panel.grid.minor = element_blank(), # element_line(linewidth = .25),
