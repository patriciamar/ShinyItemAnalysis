...
#' @aliases DDplot
#'
#' @description Plots difficulty and (generalized) discrimination or criterion validity for items of the multi-item
...
#'
#' @param data numeric: binary or ordinal data \code{matrix} or \code{data.frame} which rows represent examinee answers
#' (\code{1} correct, \code{0} incorrect, or ordinal item scores) and columns correspond to the items.
#' @param item.names character: the names of items. If not specified, the names of \code{data} columns are used.
#' @param discrim character: type of discrimination index to be calculated. Possible values are \code{"ULI"} (default),
#' \code{"RIT"}, \code{"RIR"}, and \code{"none"}. See \strong{Details}.
#' @param k numeric: number of groups to which data may be divided by the total score to estimate discrimination
...
#' @param maxscore numeric: maximal scores of items. If single number is provided, the same maximal score is used for
#' all items. If missing, vector of achieved maximal scores is calculated and used in calculations.
#' @param minscore numeric: minimal scores of items. If single number is provided, the same maximal score is used for
#' all items. If missing, vector of achieved maximal scores is calculated and used in calculations.
#' @param bin logical: should the ordinal data be binarized? Deafult value is \code{FALSE}. In case that \code{bin = TRUE},
...
#' @param cutscore numeric: cut-score used to binarize \code{data}.
#' If numeric, the same cutscore is used for all items. If missing, vector of maximal scores is used in calculations.
#' @param average.score logical: should average score of the item be disaplyed instead of difficulty? Default
#' value is \code{FALSE}. See \strong{Details}.
#' @param thr numeric: value of discrimination threshold. Default value is 0.2. With \code{thr = NULL}, no horizontal
#' line is displayed in the plot.
#' @param criterion numeric or logical vector: values of criterion. If supplied, \code{disrim} argument is ignored
#' and item-criterion correlation (validity) is displayed instead. Default value is \code{"none"}.
#' @param val_type character: criterion validity measure. Possible values are \code{"simple"} (correlation between
